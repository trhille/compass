...
class Forward(Step):
    ...
    def __init__(self, test_case, dt):
        """
        Create a new step

        Parameters
        ----------
        test_case : compass.landice.tests.time_convergence.convergence_test_case.ConvergenceTestCase
        ...
        super().__init__(test_case=test_case,
                         name='{}yr_forward'.format(dt),
                         subdir='{}yr/forward'.format(dt))

        self.dt = dt

        self.add_namelist_file(
            'compass.landice.tests.time_convergence', 'namelist.forward')

        self.add_streams_file('compass.landice.tests.time_convergence',
                              'streams.forward')
    ...
    def run(self):
        ...
        self.update_streams_at_runtime(
            'compass.landice.tests.time_convergence',
    ...
    def get_dt_duration(self):
        ...
        config = self.config
        duration = config.getint('time_convergence', 'duration')
        dur_sec = duration * sec_in_yr
        target_velocity = config.getfloat('time_convergence',
                                          'target_velocity')
        resolution = config.getint('time_convergence', 'resolution')
        # calculate dt in s for the resolution in km and velo in m/yr
        # apply ceil to ensure dt * nt actually exceeds duration
        dt_resolution_cfl = math.ceil(resolution * 1000.0 / target_velocity *
                                      sec_in_yr)
        nt_resolution = math.ceil(dur_sec / dt_resolution_cfl)
        dt_resolution = math.ceil(dur_sec / nt_resolution)
        print(f'max res: nt={nt_resolution}, dt={dt_resolution}, '
              f'err={abs(dur_sec - nt_resolution * dt_resolution) / dur_sec}')
    ...
    def _get_resources(self):
        config = self.config
        dt = self.dt
        self.ntasks = config.getint('time_convergence',
                                    f'{dt}yr_ntasks')
        self.min_tasks = config.getint('time_convergence',
                                       f'{dt}yr_min_tasks')
